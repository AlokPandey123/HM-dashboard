...
const roleConfig: Record<string, { label: string; color: string; bg: string }> = {
  superadmin: { label: 'Super Admin', color: 'text-purple-700', bg: 'bg-purple-100' },
...
export function Header() {
  ...
  const pageLabel = routeLabels[location.pathname] ?? 'HMS Portal';
  const role = roleConfig[user?.role ?? 'admin'];
  ...
  return (
    <header className="bg-white border-b border-slate-100 px-6 h-16 flex items-center justify-between shrink-0 shadow-sm">
      ...
      <div className="flex items-center gap-2 text-sm">
        <span className="text-slate-400 font-medium">HMS Portal</span>
      ...
      <div className="flex items-center gap-3">
        {/* Notification bell */}
        <button className="relative w-9 h-9 rounded-xl bg-slate-50 hover:bg-slate-100 border border-slate-200 flex items-center justify-center text-slate-500 hover:text-slate-700 transition-colors">
          <Bell size={16} />
        </button>
