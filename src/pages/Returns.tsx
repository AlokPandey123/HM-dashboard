...
import { RotateCcw, Info, CheckCircle, Clock, XCircle, TrendingDown, Filter, Loader2, ThumbsUp, ThumbsDown, ChevronLeft, ChevronRight } from 'lucide-react';
...
export function Returns() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const r = await api.get('/returns', { params: { status: statusFilter || undefined, page, limit: 20 } });
  ...
  const loadBills = async () => {
    const res = await api.get('/billing', { params: { limit: 100 } });
    setBills(res.data.data.bills.filter((b: Bill) => b.items?.some((i: { itemType: string }) => i.itemType === 'medicine')));
  };
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    if (returnItems.length === 0) { alert('No medicine items to return'); return; }
    setSaving(true);
    try {
    ...
    <div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-slate-400 text-sm flex items-center gap-1.5 mt-0.5">
            <RotateCcw size={13} /> 20% deduction applied on all returns
          </p>
        </div>
        <PermissionGuard module="returns" action="create">
          <button onClick={async () => { await loadBills(); setModal(true); }}
      ...
      <div className="flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-5">
        ...
        <p className="text-sm text-amber-800">
          ...
          20% deduction is applied on the selling price of returned medicines. Stock is restored automatically upon admin approval.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-100">
            <tr>
              {['Return ID', 'Patient', 'Bill', 'Return Amt', 'Deduction (20%)', 'Status', 'Reason', 'Actions'].map(h => (
              ...
              return (
                <tr key={r._id} className="hover:bg-slate-50/60 transition-colors">
                  <td className="px-4 py-3.5">
                    <span className="font-mono text-xs font-semibold text-blue-700 bg-blue-50 px-2 py-1 rounded-lg">{r.returnId}</span>
                  </td>
                  <td className="px-4 py-3.5">
                    <p className="font-semibold text-slate-800 text-sm">{r.patient?.name}</p>
                  ...
                  <td className="px-4 py-3.5">
                    {r.status === 'pending' && (
                      <PermissionGuard module="returns" action="update">
                        <div className="flex gap-2">
                          <button onClick={() => handleAction(r._id, 'approve')}
                            className="flex items-center gap-1 text-xs font-medium text-emerald-600 hover:text-emerald-800 border border-emerald-200 hover:bg-emerald-50 px-2 py-1.5 rounded-lg transition-colors">
                            <ThumbsUp size={11} /> Approve
                          </button>
                          <button onClick={() => handleAction(r._id, 'reject')}
                            className="flex items-center gap-1 text-xs font-medium text-red-500 hover:text-red-700 border border-red-200 hover:bg-red-50 px-2 py-1.5 rounded-lg transition-colors">
      ...
      <Modal isOpen={modal} onClose={() => setModal(false)} title="Create Return Request" size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-1.5">Select Original Bill (medicine bills only)</label>
          ...
          {selectedBill && returnItems.length > 0 && (
            <div>
              <label className="block text-xs font-semibold text-slate-600 uppercase tracking-wide mb-2">Medicine Items to Return</label>
                  ...
                  <div key={idx} className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-100 rounded-xl">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <input type="number" value={item.quantity} min="1"
                        ...
                        className="w-16 border border-slate-200 rounded-lg px-2 py-1.5 text-xs text-center focus:outline-none focus:ring-2 focus:ring-blue-500" />
                    </div>
